...
data$phealth <- ifelse(data$phealth < 4, 0, 1)
...
plot(out_binary$avg_indirect)
plot(out_binary$indv_direct[,1])
plot(out_binary$indv_indirect[,1])

# Projections GAM
proj_gam_zeta <- projection_gam(data, out_binary$indv_direct)
proj_gam_delta <- projection_gam(data, out_binary$indv_indirect)

hist(proj_gam_zeta[,1])
hist(proj_gam_delta[,1])

# Projections Tree
proj_tree_zeta <- projection_tree(data, model_m, out_binary$indv_direct)
proj_tree_delta <- projection_tree(data, model_m, out_binary$indv_indirect)

hist(proj_tree_zeta[,1])
hist(proj_tree_delta[,1])

# Actual delta, zeta
hist(out_binary$avg_direct)
hist(out_binary$avg_indirect)

hist(out_binary$indv_direct[,1])
hist(out_binary$indv_indirect[,1])
...
zeta_quantiles <- get_quantiles(out_binary$indv_direct, probs = c(0.025, 0.5, 0.975))
delta_quantiles <- get_quantiles(out_binary$indv_indirect, probs = c(0.025, 0.5, 0.975))
