...
bart_mediate <- function(data, model_m, model_y, pi_hat, m0_hat, m1_hat, mediator_name, outcome_name, treat_name, n_iter, burnin){
  ...
  for(i in 1:n_iter) {
    ...
    mu_m  <- forest_mu_m$do_gibbs(X_mu_m, m - A * tau, X_mu_m, 1)
    tau <- forest_tau$do_gibbs(X_m[A == 1,], m[A == 1] - mu_m[A == 1], X_m, 1)
    ...
    R <- (Y_scale - mu_y - A * zeta) / m
    d <- forest_d$do_gibbs_weighted(X_y, R, m^2, X_y, 1)
...
out_var_coef <- bart_mediate(data, model_m, model_y, pi_hat, m0_hat, m1_hat, 'phealth', 'logY', 'smoke', 8000, 4000)
# out_var_coef <- readRDS("Data/out_var_coef.rds")
