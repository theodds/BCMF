do_simulation_bart <- function(data, i_train, i_test,
  ...
  for (i in 1:n_reps) {
    ...
    if (file.exists(file_name_indv_bb) & file.exists(file_name_subgroup_bb) &
      ...
      indv_zeta_bb   <- zeta_test_bb
      indv_delta_bb <- delta_test_bb
      ...
      indv_zeta_catch_bb <- unlist(lapply(1:length(indv_zeta_true_bart[i_test]),
                                          function(i) (indv_zeta_true_bart[i_test][i] >= indv_zeta_interval_bb[i,1]) &
                                            (indv_zeta_true_bart[i_test][i] <= indv_zeta_interval_bb[i,2])))
      indv_delta_catch_bb <- unlist(lapply(1:length(indv_delta_true_bart[i_test]),
                                           function(i) (indv_delta_true_bart[i_test][i] >= indv_delta_interval_bb[i,1]) &
                                             (indv_delta_true_bart[i_test][i] <= indv_delta_interval_bb[i,2])))
      
      subgroup_zeta_catch_bb <- unlist(lapply(1:nrow(subgroup_zeta_true_bart),
                                              function(i) (subgroup_zeta_true_bart[i,2] >= subgroup_zeta_interval_bb[i,1]) &
                                                (subgroup_zeta_true_bart[i,2] <= subgroup_zeta_interval_bb[i,2])))
      subgroup_delta_catch_bb <- unlist(lapply(1:nrow(subgroup_delta_true_bart),
                                               function(i) (subgroup_delta_true_bart[i,2] >= subgroup_delta_interval_bb[i,1]) &
                                                 (subgroup_delta_true_bart[i,2] <= subgroup_delta_interval_bb[i,2])))
      
      
      # Saving
      # Individual
      indv_df_bb_i <- data.frame(seed = rep(seeds[i], nrow(data_test)),
      ...
      # Subgroup
      subgroup_df_bb_i <- data.frame(seed = rep(seeds[i], length(subgroup_labels)),
      ...
      # Average
      avg_df_bb_i <- data.frame(seed = seeds[i],
      ...
      indv_zeta_lb   <- zeta_test_lb
      indv_delta_lb <- delta_test_lb
